...
<br><b>③ 点开详情页</b><br>
<sub>完整 Markdown 正文 —— 这正是必须传 schedule_id 的原因</sub>
</td>
</tr>
</table>

> **测试环境**：HarmonyOS **7.0.0.107**，**仅测试了手机端**（未验证平板 / 折叠屏 / 手表等形态）。
> 截图中的「示例」卡片为测试数据，非真实任务记录。
> Markdown 支持范围见 [兼容性](#兼容性) 一节的渲染实测表。
...
| `>` 引用块 | ✅ | 左侧竖线样式正常 |
| `` `行内代码` `` | 未单独验证 | 建议少量使用 |
| **表格 `\| a \| b \|`** | ❌ **不渲染** | **表格语法不会被解析**，会退化成纯文本逐行显示，表头与分隔行也会原样出现。**请改用有序/无序列表表达二维数据。** |
| 代码块 ```` ``` ```` | 未单独验证 | 长代码块在窄屏上可读性差，建议只放结论 |

**实践建议**：卡片正文优先用「标题 + 列表 + 加粗 + 引用」这四种已验证的语法，
把表格拆成一节一个列表，例如：
...
> 表格这个结论来自真机截图（负一屏详情页把表格显示成了逐行文本）。
> 如果后续 HarmonyOS 版本补上了表格渲染，欢迎提 Issue 更正。
